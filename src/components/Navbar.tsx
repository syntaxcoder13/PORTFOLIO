...
const Navbar = () => {
  ...
  const handleScrollTo = (e: React.MouseEvent<HTMLAnchorElement>, href: string) => {
    ...
    if (lenis) {
      ...
      const triggerId = href === '#about' ? 'about-pin' :
  ...
  useEffect(() => {
    ...
    // Light-background sections by id
    const LIGHT_IDS = new Set(['about', 'achievements']);

    const checkTheme = () => {
      const navH = (navbarRef.current?.offsetHeight ?? 56) + 2;
      // Sample a point just below the navbar at the horizontal center
    ...
    // Run immediately and on every scroll tick
    checkTheme();
  ...
  // Pause Lenis scrolling when mobile menu is active
  useEffect(() => {
    const lenis = (window as any).lenis;
    if (menuOpen) {
      lenis?.stop();
  ...
  return (
    <>
      ...
      >
        <div className="flex items-center justify-between px-6 md:px-10 py-2.5 md:py-3 max-w-7xl mx-auto w-full">
          {/* Logo */}
          <a
            href="#hero"
            onClick={(e) => {
            ...
            {NAV_LINKS.map((link) => (
              <li key={link.label}>
                <a
                  ...
                  style={{ fontFamily: 'Helvetica, Arial, sans-serif' }}
                  className={`text-xs sm:text-[13px] font-semibold transition-colors duration-500 ${isLight
          ...
          <div className="flex items-center gap-4">
            ...
            <a
              ...
              style={{ fontFamily: 'Helvetica, Arial, sans-serif' }}
              className={`hidden md:inline-flex items-center gap-1.5 rounded-full border px-6 py-2.5 text-xs sm:text-[13px] font-bold transition-all duration-500 hover:scale-[1.02] active:scale-[0.98] ${isLight
